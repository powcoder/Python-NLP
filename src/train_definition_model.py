...
def evaluate_model(sess, data_dir, input_node, target_node, prediction,
  ...
  all_rank = []
  for gloss, h in res[0]:
    model_preds = sess.run(prediction, feed_dict={input_node: gloss})
    if out_form == "softmax":
      sims = model_preds
    else:
      sims = 1 - dist.cdist(model_preds, embs, metric="cosine")
      sims = np.nan_to_num(sims)

    ranked_ids = sims.argsort(axis = 1)[:, ::-1]
    ranks = [np.where(ranked_ids[i] == h[i])[0][0] for i in range(h.shape[0])]
    all_rank = all_rank + ranks

  for i in range(len(all_rank)):
    print("instance %3d, rank %5d" %(i, all_rank[i]))

  median_rank = np.median(all_rank)

  print("Median rank: %f" % median_rank)
